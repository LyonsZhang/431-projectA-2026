...
This page describes what to do if you want to use something other than NHANES data for Project A. If you want to use NHANES data, then you should [visit this page instead](data2.qmd).
...
To encourage people to use data other than NHANES, there is a four-point bonus in the final project A grade available for all projects which use non-NHANES data.

## If you're using data from another source

If you don't want to use NHANES data, you will need to obtain Dr. Zhang's approval through the registration form. Here are the data specifications.

0. If it is beyond your mind to fins any other data, here are two recommendations [Cancer Genomics](https://www.cbioportal.org/), [Physiologic study](https://physionet.org/) and [Cardio Stats from NHANES](https://jhs-hwg.github.io/cardioStatsUSA/reference/nhanes_data.html).

1. The data must be **freely available** to all, and there must be no risk associated with your using the data for this project of any kind. Your use of the data for this project must not be subject to IRB approval, or the approval of anyone other than you (so, for example, if you would also need the approval of a principal investigator to use the data, that won't work for Project A.) 
    - There can be no protected health information or protected information or privacy risk of any kind involved with the data.

2. Dr. Zhang will need to see your source for the data in its entirety. You will need to be able to provide a link to a web page from which you (and Dr. Zhang and anyone else) can **download the raw data** as part of your project submission.
<!--[registration for the project](register.qmd) in mid-November.-->
...
4. The data must not be hierarchical, so everything must be measured at the subject level.
    ...
    - The data you select must in all ways be suitable for the analyses required in Project A.
...
7. Dr. Zhang has a strong preference for data that describe individual people or animals, as opposed to other types of "subjects". Who the subjects (rows) of your data are must be completely clear. 
<!--No genomics data, either, in Project A - Dr. Zhang is insufficiently familiar with that sort of data.-->

8. Dr. Zhang can refuse to let you use a data set for any reason at all, and this includes the reason that he's tired of the data set.
